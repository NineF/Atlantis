...
public class HTTPServer {
     ...
     * 通过注解调用
     */
    public static class TimeContextHandler {

        @Context(value = "/api/time/one")
        private int execute(Request req, Response resp) {
        ...
        @Context(value = "/api/time/two")
        private int execute1(Request req, Response resp) {
            long now = System.currentTimeMillis();
            resp.getHeaders().add("Content-Type", "text/plain");
            try {
                System.out.println("调用到这里1");
                resp.send(200, String.format("%tF %<tT", now));
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public static void main(String[] args) {
        try {
            ...
            host.addContexts(new TimeContextHandler());
            server.start();
